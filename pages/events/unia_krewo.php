<?PHP  header("Content-Type: text/html; charset=utf-8");?>
<!DOCTYPE html>
<html lang="ru">
  <head>
  <!-- ***Title*** -->
    <title>Кревская уния. История Беларуси</title>
<!-- ***Meta*** -->  
    <meta name="description" content="Соглашение о династическом союзе между ВКЛ и Польшей, по которому литовский великий князь Ягайло, 
	вступив в брак с польской королевой Ядвигой, провозглашался польским королём.
	" />
	<meta name="keywords" itemprop="keywords" content="кревская уния, 
	история беларуси кратко в датах, основные события история Беларуси, история ВКЛ, ВКЛ, история Беларуси, Краткая история Беларуси, История беларуси кратко" />
	<meta property="og:image" content="http://history-belarus.by/images/img-events/unia_krewo/unia_title.jpg" />

<!-- ***Include Header*** -->
<? include ("../_include/header_events.php"); ?>
  
  <!-- ***Content & Sidebars*** -->
  <div class="container-fluid">
  

      <!-- ***Sidebar Left - include CurrentDir*** -->
<? include ("../_include/dir_events.php"); ?>

	
    <!-- ***Page Content*** -->
    <div class="col-md-8">
                  
    <!-- ***Top list [A-Z] *** -->
    <? include ("../_include/top_list_events.php"); ?>
  
		<h3>1385 Кревская Уния</h3>
				
		<p class="img-float-left">
		<a href="../../images/img-events/unia_krewo/unia_krewo.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img  alt="Кревская уния" src="../../images/img-events/unia_krewo/unia_krewo_thumb.jpg" height="140">
		</a>
		</p>
		<p class="img-float-right">
		<a href="../../images/img-places/krevo/krevo_place.jpg" class="highslide"
        onclick="return hs.expand(this)">
			<img alt="Кревская уния" src="../../images/img-places/krevo/krevo_place.jpg" height="140" >
			</br><small>Крево</small>
			</a>
		</p>

		<p>
		Кревская уния 14 августа 1385 года, подписанная в замке <a href="../places/krevo.php">Крево</a> — 
		соглашение о династическом союзе между <a href="../terms/wkl.php">Великим княжеством Литовским</a> и Польшей, 
		по которому литовский великий князь <a href="../figures/jagiello.php">Ягайло</a>, 
		вступив в брак с польской королевой Ядвигой, провозглашался польским королём.
		</p>
		<p>
		Войска, законодательство и судебная система, а также казна (включая денежную эмиссию) оставались раздельными, 
		сохранялась и граница между государствами с взиманием таможенных сборов.
		</p>
		
		<p> 
		1377 Смерть великого князя <a href="../figures/olgerd.php">Ольгерда</a> вызвала внутренний разлад в ВКЛ.
		</br>1381 Начался первый этап <a href="../events/war_civil_1381-84_1389-92.php">Гражданской войны</a> (<a href="../figures/vitovt.php">Витовт</a> vs Ягайло), в ходе которой оба противника старались заручиться помощью Тевтонского Ордена.
		</br>1384 Закончился первый этап Гражданской войны. За оказанную помощь Ягайло обязался (по Дубисскому договру) передать Ордену 
		<a href="../places/samogitia.php">Жмудь</a> 
		и принять католичество. Однако после устранения опасности с востока (Тохтамыш сжег Москву), появилась возможность отобрать Жмудь обратно. 
		</p>
		<p>
		В 1384 году в Польше королевой стала Ядвига, которой магнаты искали мужа, способного помочь отражать экспансию Тевтонского Ордена.
 		</p>
		<p>
		1385 Уния Великого Княжества и Короны тогда отвечала интересам обеих сторон. Она позволяла объединить силы двух государств против общего 
		врага — крестоносцев. Немецкая экспансия в Польше достигла угрожающих масштабов. 
		</p>
		<p>
		Полякам важно было обеспечить спокойствие своих границ и со стороны Великого Княжества Литовского, 
		так как литвины только за один поход в 1376 году вывели из Польши 23 тысячи пленных.
		</p>
		
		<div class="col-md-12">
		
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-events/unia_vilno/wkl_polska_1386_1434.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Кревская уния" src="../../images/img-events/unia_vilno/wkl_polska_1386_1434_thumb.jpg" height="160">
		</br><small>ВКЛ 1386-1434</small>
		</a>
		</div>
		
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-events/unia_liublin/unia_table.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Кревская уния" src="../../images/img-events/unia_liublin/unia_table_thumb.jpg" height="160">
		</br><small>Хронология уний
		</br>ВКЛ-Польша</small>
		</a>
		</div>
		
		
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-events/unia_krewo/ordensgebiete.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Кревская уния" src="../../images/img-events/unia_krewo/ordensgebiete_thumb.jpg" height="160">
		</br><small>XIV-XV в. Земли Deutschen Ordens</small>
		</a>
		</div>
		
		</div>
		
		<p>
		Подписание Кревской унии, хотя и вызвало волну недовольства части знати ВКЛ, 
		стало этапом к прекращению борьбы между двумя государствами за юго-западные <a href="../download/zemie_ruskie.php">Русские земли</a> 
		(Галицию, Волынь, Холмщину) и способствовало расширению их границ до побережья Чёрного моря.
		</p>
		
		
		</br></br></br>
		<p><small>
		be-x-old.wikipedia.org</br>
		be.wikipedia.org</br>
		pl.wikipedia.org</br>
		uk.wikipedia.org</br>
		ru.wikipedia.org</br>
		</small></p>
		
	</div>
	
	<!-- ***Sidebar Right*** -->

	<? include ("../_include/right_sidebar_events.php"); ?>

	
</div>

<!-- ***Include Footer*** -->
<? include ("../_include/footer.php"); ?>