<?PHP  header("Content-Type: text/html; charset=utf-8");?>
<!DOCTYPE html>
<html lang="ru">
  <head>
   
<!-- ***Title*** -->
 	<title>Рогволод, Рогнеда и Изяслав. Рогволодовичи</title>

<!-- ***Meta*** -->
	<meta name="description" content="Короткая история о Рогволоде — первом князе Полоцком, его дочери Рогнеде и внуке Изяславе, 
	о динстии Изяславичей - внуков Рогволожих 
	" />
  	<meta name="keywords" itemprop="keywords" content="Рогволод, Рогнеда, Изяслав, Рогволодовичи, 
	  история беларуси, история белоруссии, история беларуси кратко в датах, известные люди беларуси, знаменитые люди беларуси"/>
	  <meta property="og:image" content="http://history-belarus.by/images/img-figures/rogvolod_rogneda/title.jpg" />

<!-- ***Include Header*** -->
<? include ("../_include/header_figures.php"); ?>
  
  <!-- ***Content & Sidebars*** -->
  <div class="container-fluid">
  

      <!-- ***Sidebar Left - include CurrentDir*** -->
<? include ("../_include/dir_figures.php"); ?>

	
    <!-- ***Page Content*** -->
    <div class="col-md-8">
                       
    <!-- ***Top list [A-Z] *** -->
    <? include ("../_include/top_list_figures.php"); ?>
  
		<h1>Рогволод, Рогнеда и Изяслав. Рогволодовичи</h1>
		<h3>Рогволод и "Рогволожи внуки"</h3>
		<p class="img-float-left">
		<a href="../../images/_img-index/blr_10ce_country_variag_way.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Рогволод, Рогнеда и Изяслав" src="../../images/_img-index/blr_10ce_country_variag_way_thumb.jpg" height="120">
		</br><small>Полоцкое княжество</small>
		</a>
		</p>
		<p class="img-float-right">
		<a href="../../images/img-terms/polotsk_principality/monet_rb.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Рогволод, Рогнеда и Изяслав" src="../../images/img-terms/polotsk_principality/monet_rb_thumb.jpg" height="120">
		</br><small>Памятная монета, РБ</small>	
		</a>
		</p>
		<p>
		Рогволод (920-978) — первый полоцкий князь, названный по имени в исторических источниках. 
		До сих пор остается почти мистической фигурой — сведений о нем и его происхождении мало: <i>"пришел из-за моря и держал власть свою в Полоцке"</i>. 
		</p>
		<p>
		Большинство историков принимают его скандинавское происхождение и возводят к нему Полоцкую княжескую династию, 
		обособляя ее от Киево-Новгородских Рюриковичей.
		</p>
		<p>
		<i>"Особое тяготение варягов к Полоцку"</i> подчеркивал А. Н. Веселовский в своей работе 
		"<a target="_blank" href="../../images/img-terms/polotsk_principality/Veselovskij_Rus_in_Wilkinasaga.djvu">Русские и вильтины в саге о Тидреке Бернском</a> (Wilkinasaga)". 
		Так же, как и <i>"необщерусскую его политическую ориентацию — у одних на руках половцы, у других Литва и Ливь"</i>. 		
		</p>
		<p>
		Повесть Временных лет, год 6488 (980) — <i>"И послал [Владимир] к Рогволоду в Полоцк сказать: "Хочу дочь твою взять себе в жены". Тот же спросил у дочери своей: "Хочешь ли за Владимира?". Она ответила: "Не хочу разуть сына рабыни, но хочу за Ярополка". Этот Рогволод пришел из-за моря и держал власть свою в Полоцке, а Туры держал власть в Турове, по нему и прозвались туровцы… И напал Владимир на Полоцк, 
		и убил Рогволода и двух его сыновей, а дочь его взял в жены."</i>		
		</p>
		<p>
		<i>"И с тех пор взимают меч Рогволожи внуки против Ярославовых внуков."</i>
		</br>Дети от брака Владимира и Рогнеды положили начало родам Изяславичей ("Рогволожи внуки" — князья Полоцкие) от старшего Изяслава и Ярославичей 
		("Рюриковичи" — князья Киевские, Новгородские, Ростовские) от Ярослава Мудрого, младшего сына. 
		</p>
		<p>
		<i>"В вековой распре Ярославичей и Изяславичей есть что-то расовое"</i> — отмечал профессор Веселовский. 
		Полоцкие князья не участвовали в 
		<a target="_blank" href="https://ru.wikipedia.org/wiki/Съезды_русских_князей">
		съездах руских князей</a> (Ярославичей). Единственный раз, после <a href="../events/battle_nemiga.php">битвы на Немиге</a>  
		(князь <a href="../figures/charadzej.php">Всеслав-Чародей</a> vs Ярославичи) 
		киевский князь Изяслав приглашал Всеслава на съезд и <i>«крест целовал: "Приди к нам, не сотворим тебе зла"»</i>. Там Всеслав был пленен, 
		затем освобожден восставшими горожанами и киевским вече посажен на Киевское княжение.
		</p>
		
	<div class="col-md-12">
	
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-terms/religion_blr/viking expeditions.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Полоцкое княжество" src="../../images/img-terms/religion_blr/viking expeditions_thumb.jpg" height="100">
		</br><small>Экспедиции викингов </small>	
		</a>
		</div>
		
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-terms/polotsk_principality/saga_ditrek.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Рогволод, Рогнеда и Изяслав" src="../../images/img-terms/polotsk_principality/saga_ditrek_thumb.jpg" height="100">
		</br><small>А. Н. Веселовский о Полоцке</small>	
		</a>
		</div>
	
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-terms/polotsk_principality/about_Polotsk_duces.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img src="../../images/img-terms/polotsk_principality/about_Polotsk_duces_thumb.jpg" height="100">
		</br><small>Рогволодовичи vs Ярославичи</small>	
		</a>
		</div>
		
	</div>
		
		<p>
		От Изяслава — внука Рогволода выводило происхождение <a href="../terms/dukes_wkl.php">Гедиминовичей</a> Русское Императорское Географическое Общество в 
		своем труде <a href="../download/kirkor.php">"Живописная Россия"</a>, 1882
		</p>

		
	<div class="col-md-12">

		<div class="col-md-6 img-float-center">
		<a href="../../images/img-figures/rogvolod_rogneda/1844_hstorie_lelevel.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Рогволод, Рогнеда и Изяслав" src="../../images/img-figures/rogvolod_rogneda/1844_hstorie_lelevel_thumb.jpg" height="100">
		</br><small>Князья Полоцкие</small>
		</a>
		</div>
		
		<div class="col-md-6 img-float-center">
		<a href="../../images/img-figures/gedimin/kirkor_litwa_duces.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img src="../../images/img-figures/gedimin/kirkor_litwa_duces_thumb.jpg" height="100">
		</br><small>Родовод Гедимина</small>	
		</a>
		</div>	
		
	</div>
	
		<h3>Рогнеда и Изяслав</h3>
		
		<p>
		Рогнеда (960-1000) — дочь князя Рогволода, князя Полоцкого.
		</p>
		<p>
		События, связанные с Рогнедой, изложены в "Повести временных лет". Запись от 1128 года говорит об этой истории, как о событии, полжившем начало вражде
		Рогволодовичеий и Ярославичей. Подробный анализ источников приведен в 
			<a target="_blank" href="../../images/img-terms/dwina_stouns/about_Polotsk_duces.pdf">работе А. В. Рукавишникова</a> (МГУ им Ломоносова). 
		</p>
		<p>
		В 980 году Рогнеда была объявлена невестой Ярополка Святославича, великого князя киевского. Брат Ярополка Владимир, в то время князь новгородский, 
		был сильно унижен Рогнедой, так как тоже сватался к ней, однако был назван "робичичем" и получил отказ: княжна считала недопустимым выйти замуж 
		за сына наложницы. 
		</p>
		<p>
		Убив Рогволода и его сынвей и взяв Рогнеду в жены насильно, Владимир увез ее в Киев.  Примерно в 987 она решилась отомстить за содеянное и убить мужа. Согласно легенде, ночью, когда Владимир спал, Рогнеда хотела заколоть его, но князь проснулся и успел отвести удар.
		</p>
		<p>
		За покушение Рогнеде грозила смертная казнь. Разгневанный Владимир приказал жене нарядно одеться и взял в руки меч, однако на крик прибежал их первенец Изяслав и стал на защиту матери также с мечом в руках. Владимир не смог убить Рогнеду на глазах сына. 
		</p>
		<p>
		По совету своих бояр: <i>"Не убивай её ради дитяти сего, но воздвигни отчину отца её, и отдай ей с сыном твоим"</i>,
		Владимир поселил Рогнеду и Изяслава в их полоцкой земле — городе, который был назван Изяславль (Заславль).
		</p>
		<p>
		Потомками Рогволода были не только полоцкие князья (Изяславовичи "Рогволожи внуки"), но и Ярославичи, так как Ярослав Мудрый несколько раз назван летописцем сыном Рогнеды, младшим братом Изяслава.
		</p>
		<p>
		Ниже приведены иллюстрации из <a href="../download/letopis_radziwil.php">Радзивилловской летописи</a> XIII века.
		</p>		
		
	<div class="col-md-12">
		
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-figures/rogvolod_rogneda/Rogvolod_swaty.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Рогволод, Рогнеда и Изяслав" src="../../images/img-figures/rogvolod_rogneda/Rogvolod_swaty_thumb.jpg" height="100">
		</br><small>Сваты Владимира у Рогволода</small>
		</a>
		</div>
		
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-figures/rogvolod_rogneda/Rahnieda_Volodymyr.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Рогволод, Рогнеда и Изяслав" src="../../images/img-figures/rogvolod_rogneda/Rahnieda_Volodymyr_thumb.jpg" height="100">
		</br><small>Покушение Рогнеды на Владимира</small>	
		</a>
		</div>
		
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-figures/rogvolod_rogneda/izaslaw_rognieda.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Рогволод, Рогнеда и Изяслав" src="../../images/img-figures/rogvolod_rogneda/izaslaw_rognieda_thumb.jpg" height="100">
		</br><small>Изяслав защищает Рогнеду</small>	
		</a>
		</div>	
		
	</div>
	
		<p>
		Легенда о гордой Рогнеде и храбром Изяславе — один из немногих летописных сюжетов тысячелетней давности, знакомый даже тем, кто никогда не интересовался историей. 
		</p>
	
	<div class="col-md-12">
	
		<div class="col-md-3 img-float-center">
		<a href="../../images/img-terms/polotsk_principality/Izjaslaw_Rahnieda.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Рогволод, Рогнеда и Изяслав" src="../../images/img-terms/polotsk_principality/Izjaslaw_Rahnieda_thumb.jpg" height="100">
		</br><small>Заславль 987</small>	
		</a>
		</div>
		
		<div class="col-md-3 img-float-center">
		<a href="../../images/img-figures/rogvolod_rogneda/rogneda_izalaw_memory.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Рогволод, Рогнеда и Изяслав" src="../../images/img-figures/rogvolod_rogneda/rogneda_izalaw_memory_thumb.jpg" height="100">
		</br><small>Рогнеда и Изяслав, 
		</br>г. Заславль</small>	
		</a>
		</div>
		
		<div class="col-md-3 img-float-center">
		<a href="../../images/img-figures/rogvolod_rogneda/izaslaw_memory.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Рогволод, Рогнеда и Изяслав" src="../../images/img-figures/rogvolod_rogneda/izaslaw_memory_thumb.jpg" height="100">
		</br><small>Изяслав, 
		</br>г. Заславль</small>	
		</a>
		</div>
		
		<div class="col-md-3 img-float-center">
		<a href="../../images/img-figures/rogvolod_rogneda/zaslawje.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Рогволод, Рогнеда и Изяслав" src="../../images/img-figures/rogvolod_rogneda/zaslawje_thumb.jpg" height="100">
		</br><small>Старое городище, 
		</br>г. Заславль</small>	
		</a>
		</div>		
		
	</div>
		
		</br></br></br>
		<p><small>
		http://inbelhist.org/rogvolod-kotoryj-prishel-i-zamorya/</br>
		be-x-old.wikipedia.org</br>
		be.wikipedia.org</br>
		pl.wikipedia.org</br>
		uk.wikipedia.org</br>
		ru.wikipedia.org</br>

		</small></p>
	</div>
	
	<!-- ***Sidebar Right*** -->

	<? include ("../_include/right_sidebar_figures.php"); ?>

	
</div>

<!-- ***Include Footer*** -->
<? include ("../_include/footer.php"); ?>