<?PHP  header("Content-Type: text/html; charset=utf-8");?>
<!DOCTYPE html>
<html lang="ru">
  <head>
  <!-- ***Title*** -->
 	<title>Стефан Баторий</title>

<!-- ***Meta*** -->
	<meta name="description" content="Он освободил Полоцк, завершил победой Ливонскую войну, 
	ввел григорианский календарь и хотел перенести столицу в Гродно.
	" />
  	<meta name="keywords" itemprop="keywords" content="Стефан Баторий,  
	  история беларуси, история белоруссии, история беларуси кратко в датах, известные люди беларуси, знаменитые люди беларуси"/>
	  <meta property="og:image" content="http://history-belarus.by/images/img-figures/batory/title.jpg" />

<!-- ***Include Header*** -->
<? include ("../_include/header_figures.php"); ?>
  
  <!-- ***Content & Sidebars*** -->
  <div class="container-fluid">
  

      <!-- ***Sidebar Left - include CurrentDir*** -->
<? include ("../_include/dir_figures.php"); ?>

	
    <!-- ***Page Content*** -->
    <div class="col-md-8">
                   
    <!-- ***Top list [A-Z] *** -->
    <? include ("../_include/top_list_figures.php"); ?>
  
		<h1>Стефан Баторий</h1>
		
		<div class="col-md-12">
		
		<p class="img-float-left" >
		<a href="../../images/img-figures/batory/1576_batory.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Стефан Баторий" src="../../images/img-figures/batory/1576_batory_thumb.jpg" height="120" >
		</br><small>Стефан Баторий, 1576</small>
		</a>
		</p>
		
		<p class="img-float-right" >
		<a href="../../images/img-figures/batory/Blaeu_1645_Transylvania.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Стефан Баторий" src="../../images/img-figures/batory/Blaeu_1645_Transylvania_thumb.jpg" height="120" >
		</br><small>Герб Батория, 1645</small>
		</a>
		</p>

		<p>
		Стефан Баторий (27 сентября 1533 - 12 декабря 1586, Гродно) — король польский и <a href="../terms/dukes_wkl.php">великий князь литовский</a> 1575-86. 
		Сын трансильванского воеводы, князь Трансильвании 1571-76. Герб Батория — три когтя/зуба дракона, которого по преданию убил его предок.
		</p>
		<p>
		На польском королевском троне стал преемником <a href="../events/confederacia_warshawa.php">Генриха Валуа</a>, 
		женившись на Анне Ягеллонке (от брака с которой Генрих отказался в пользу шансов занять французский трон). 
		Вслед за Генрихом, так же как и все последующие короли, подписал <a href="../events/confederacia_warshawa.php">"Генриховы артикулы"</a>, 
		подтверждавшие верховенство <a href="../terms/shliahta.php">"шляхетской демократии"</a>.
		</p>
		
		</div>
		
		<div class="col-md-12">
		
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-figures/batory/1586_taler_Batory.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Стефан Баторий" src="../../images/img-figures/batory/1586_taler_Batory_thumb.jpg" height="100" >
		</br><small>Талер Батория, 1586</small>
		</a>
		</div>
		
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-figures/batory/taler.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Стефан Баторий" src="../../images/img-figures/batory/taler_thumb.jpg" height="100" >
		</br><small>Монета Трансильвании, 1593</small>
		</a>
		</div>
		
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-figures/batory/1674_Sanson.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Стефан Баторий" src="../../images/img-figures/batory/1674_Sanson_thumb.jpg" height="100" >
		</br><small>Трансильвания, Sanson 1674</small>
		</a>
		</div>
		
		</div>
		
		<p>
		Правление Стефана Батория пришлось на годы <a href="../events/war_livonia.php">Ливонской войны</a> с Московским царством. 
		При нем был освобожден Полоцк, войска дошли до Пскова и Старой Русы, 
		были ликвидированы все завоевания Ивана Грозного в Ливонии. Территория Ливонской конфедерации была разделена между Речью Посполитой, Швецией и Данией.
		</p>
		<p>
		Король управлял страной без знания языков своих подданных (пользовался латынью). Однако в 
		<a target="_blank" href="http://niab.by/unikdocs/">Национальном архиве</a> РБ сохранилась грамота Батория 
		на <a href="../terms/blr_mova.php">старобелорусском</a> (государственном языке ВКЛ). 
		Что характерно — она адресована справцу <i>венгерских</i> войск.
		</p>
		
		<div class="col-md-12 img-float-center">
		
		<div class="col-md-3">
		<a href="../../images/img-events/war_livonia/livon1558.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Ливонская война" src="../../images/img-events/war_livonia/livon1558_thumb.jpg" height="100">
		</br><small>Ливонская война</small>
		</a>
		</div>
		
		<div class="col-md-3">
		<a href="../../images/img-terms/blr_flag/Siege_of_Polotsk_in_1579.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Флаг Беларуси БЧБ" src="../../images/img-terms/blr_flag/Siege_of_Polotsk_in_1579_thumb.jpg" height="100"> 
		</br><small>Осада Полоцка 1579 года</small>
		</a>
		</div>
		
		<div class="col-md-3">
		<a href="../../images/img-events/war_livonia/Jan_Matejko-Batory_pod_Pskowem.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Ливонская война" src="../../images/img-events/war_livonia/Jan_Matejko-Batory_pod_Pskowem_thumb.jpg" height="100">
		</br><small>Стефан Баторий под Псковом</small>
		</a>
		</div>
		
		<div class="col-md-3">
		<a href="../../images/img-terms/blr_mova/1581_gramota_batory.jpg" class="highslide"
        	onclick="return hs.expand(this)">      	
			<img src="../../images/img-terms/blr_mova/1581_gramota_batory_thumb.jpg" height="100"/>
			</br><small>1581, грамота Стефана Батория</small>
		</a>
		</div>
		
		</div>
	
		<p>
		Стефан Баторий много сделал для образовательной системы. Активно поддерживал <a href="../terms/jesuity.php">иезуитов</a>, чей образовательный стандарт был лучшим в Европе. 
		При нем  были основаны иезуитские коллегиумы в Люблине, Полоцке, Риге, Калише,  Несвиже, Львове и Дерпте. 
		Его привилеем Виленская иезитская коллегия преобразована в Академию (Университет). 
		Для основания коллегиумов в Гродно и Бресте не хватило кадровых ресурсов у провинции ордена и времени жизни у короля.
		</p>
		<p>
		В 1582 Стефан Баторий ввел в <a href="../terms/rzeczpospolita.php">Речи Посполитой</a> григорианский календарь, которым мы пользуемся сегодня 
		(в России он был введен в 1918 году). 
		</p>
		<p>
		В связи с нацеленностью внешней политики на восток Баторий стремился развивать инфраструктуру государственного управления 
		в границах ВКЛ, планировал перенести столицу Речи Посполитой в <a href="../places/grodno.php">Гродно</a>, 
		где перестроил королевский замок, реконструировал <a href="../places/fara_grodno.php">Фару Витовта</a> и провел последние годы жизни. 
		</p>
		
		<div class="col-md-12">
		
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-places/fara_grodno/1568_Vera Designatio Urbis in Littavia Grodnae.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Фара Витовта Гродно" src="../../images/img-places/fara_grodno/1568_Vera Designatio Urbis in Littavia Grodnae_thumb.jpg" height="100" >
		</br><small>Гродно, 1568</small>
		</a>
		</div>
		
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-figures/batory/fara_vitovt.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Стефан Баторий" src="../../images/img-figures/batory/fara_vitovt_thumb.jpg" height="100" >
		</br><small>Фара Витовта, Гродно</small>
		</a>
		</div>
		
		<div class="col-md-4 img-float-center">
		<a href="../../images/img-figures/batory/zamak.jpg" class="highslide"
        onclick="return hs.expand(this)">
		<img alt="Стефан Баторий" src="../../images/img-figures/batory/zamak_thumb.jpg" height="100" >
		</br><small>Старый замок, Гродно</small>
		</a>
		</div>
		
		</div>
		
		<p>
		<i>"Из внутренних реформ, введенных Баторием в Польше, особенно заслуживает внимания устройство запорожских казаков, 
		которым он дал правильную организацию, наделил землями, позволил самим выбирать гетмана и все военное начальство, 
		оставляя за королем право наделения гетмана знаменем, "булавой" и печатью и утверждения его после принятия присяги на верность. 
		Желая восстановить не только политический, но и духовный мир в Польше, Баторий старался привести к соглашению с католичеством диссидентов (не католиков),
		 для чего не прибегал, однако, никогда к насилию, но старался действовать гуманно и мирно."</i>
		 </br><small>Энциклопедический словарь Брокгауза и Ефрона</small> 		
		</p>
		
		</br></br></br>
		<p><small>
		be-x-old.wikipedia.org</br>
		be.wikipedia.org</br>
		pl.wikipedia.org</br>
		uk.wikipedia.org</br>
		ru.wikipedia.org</br>
		</small></p>
		
	</div>
	
	<!-- ***Sidebar Right*** -->

	<? include ("../_include/right_sidebar_figures.php"); ?>

	
</div>

<!-- ***Include Footer*** -->
<? include ("../_include/footer.php"); ?>