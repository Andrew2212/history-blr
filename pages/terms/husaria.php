<?PHP  header("Content-Type: text/html; charset=utf-8");?>
<!DOCTYPE html>
<html lang="ru">
  <head>
    
<!-- ***Title*** -->
 	<title>Крылатые гусары ВКЛ. История Беларуси</title>

<!-- ***Meta*** -->
    <meta name="description" content="Гусария, или крылатые гусары - элитная кавалерия Речи Посполитой, 
	действовавшая на полях сражений с середины XVI века до середины XVIII века.
	Гусария была подразделением народного ауторамента (наёмного войска местной военной традиции, набирали по принципу товариществ). 
	Товарищ приводил с собой почт (драб - пеший, пехотинец, пахол(и)к - слуга, оруженосец), коней и челядь.
	" />
    <meta name="keywords" itemprop="keywords" content="крылатые гусары вкл, гусария, 
	 Беларусь, белорусский этнос, религия в Беларуси, белорусы, краткая история Беларуси, история Беларуси, история Белоруссии, 
	 История беларуси кратко в датах" />
	 <meta property="og:image" content="http://history-belarus.by/images/img-terms/husaria/nesviz_1930_radziwill_palac.jpg" />


<!-- ***Include Header*** -->
<? include ("../_include/header_terms.php"); ?>
  
  <!-- ***Content & Sidebars*** -->
  <div class="container-fluid">
  

      <!-- ***Sidebar Left - include CurrentDir*** -->
<? include ("../_include/dir_terms.php"); ?>

	
    <!-- ***Page Content*** -->
    <div class="col-md-8">
        
    <!-- ***Top list [A-Z] *** -->
    <? include ("../_include/top_list_terms.php"); ?>
    
		<h3>Крылатые гусары ВКЛ</h3>
		
		<p class="img-float-left">
		<a target="_blank" href="../../images/img-terms/husaria/mir_castle_husaria.jpg">
		<img src="../../images/img-terms/husaria/mir_castle_husaria_thumb.jpg" height="160">
		</br><small>Доспехи, Мирский замок</small>
		</a>
		</p>
		<p class="img-float-right">
		<a target="_blank" href="../../images/img-terms/husaria/dospech_radziwill.jpg">
		<img src="../../images/img-terms/husaria/dospech_radziwill_thumb.jpg" height="160">
		</br><small>Рыцарский доспех 
		</br>Радзивилла Черного, 1550</small>
		</a>
		</p>
				
		<p>
		Гусария, или крылатые гусары — элитная кавалерия <a href="rzeczpospolita.php">Речи Посполитой</a>, действовавшая на полях сражений 
		с середины XVI века до середины XVIII века. 
		</p>		
		<p>
		Гусария была подразделением народного ауторамента (наёмного войска местной военной традиции, 
		набирали по принципу товариществ). Завербованный товарищ приводил с собой почт 
		(<i>драб</i> — пеший, пехотинец, <i>пахол(и)к</i> — слуга, оруженосец), коней и челядь. 
		</br><small>[ см. <a href="../download/spis_voiska.php">Перепись Войска Литовского</a>, 1567 ]</small>
		</p>		
		<p>		 
		Кавалерия, и самая её боеспособная часть — гусария — являлись основной военной силой Речи Посполитой. 
		Длительное время гусария не имела себе равных в Европе, и её атаки не раз приносили победу 
		Королевству Польскому и <a href="wkl.php">Великому княжеству Литовскому</a> — как в 
		<a href="../events/battle_kircholm.php">битве при Кирхгольме</a>, где 4-х тысячная армия ВКЛ под командованием 
		<a href="../figures/hodkewich.php">Яна Ходкевича</a> разгромило 11-ти тысячное шведское войско.
		</p>
		<p>
		Первыми гусарами были сербы (легкая конница без доспехов), поступившие на службу польского короля в конце XV века. 
		К середине XVI века гусария трансформировалась в тяжелую конницу, близкую западноевропейской рыцарской традиции.
		</p>		
		<p>
		На внешний вид крылатых гусар оказали влияние турецкие "дели" (безумные) — они обычно набирались из балканских народов, 
		подвластных Османской империи. В 1577 году король и в. князь 
		<a href="../figures/batory.php">Стефан Баторий</a> произвёл стандартизацию гусарского вооружения и снаряжения. 
		</p>
		
	<div class="col-md-12" >
	
		<div class="col-md-3 img-float-center">
		<a target="_blank" href="../../images/img-terms/husaria/turkish-deli.jpg">
		<img src="../../images/img-terms/husaria/turkish-deli_thumb.jpg" height="120">
		</br><small>Турецкие "дели", XVI</small>	
		</a>
		</div>
		
		<div class="col-md-3 img-float-center">
		<a target="_blank" href="../../images/img-terms/husaria/autorament.jpg">
		<img src="../../images/img-terms/husaria/autorament_thumb.jpg" height="120">
		</br><small>Народный ауторамент</small>	
		</a>
		</div>
		
		<div class="col-md-3 img-float-center">
		<a target="_blank" href="../../images/img-terms/husaria/nesviz_1930_radziwill_palac.jpg">
		<img src="../../images/img-terms/husaria/nesviz_1930_radziwill_palac_thumb.jpg" height="120">
		</br><small>Оружейная Радзивиллов
		</br>Несвиж, 1930</small>	
		</a>
		</div>
		
		<div class="col-md-3 img-float-center">
		<a target="_blank" href="../../images/img-terms/husaria/kluszyn_1610.jpg">
		<img src="../../images/img-terms/husaria/kluszyn_1610_thumb.jpg" height="120">
		</br><small>Гусарская хоругвь 
		</br>в битве при Клушине</small>	
		</a>
		</div>
									
	</div>
			
		<p>
		150 лет гусария была грозной силой. XVII век стал "золотым" веком крылатых гусар. 
		</br>Гусарские хоругви сыграли решающую роль в ряде крупных сражений: под Кирхольмом (1605) и Клушиным (1610), Хотином (1621 и 1773) и Веной (1683).
		Наибольшей численности в своей истории гусария достигла в 1621 г. (8000 всадников). 
		 </br>Секрет боевой эффективности гусар был достаточно прост — они вернули на поле боя классический таранный удар, 
		 применявшийся рыцарской кавалерией. 
		 </br>Но к XVIII веку огнестрельное оружие вытеснило тяжелую кавлерию. В 1775 году сейм упразднил формирования крылатых гусар.
		</p>
		<p class="img-float-center">
		<a target="_blank" href="../../images/img-terms/blr_gerb/gusaria_signs.jpg">
		<img src="../../images/img-terms/blr_gerb/gusaria_signs.jpg" height="120">
		</br><small>Белоруский и Гродненский 
		</br>гусарские полки, XIX</small>	
		</a>
		</p>
		<p>
		Традиции гусарии ВКЛ были продолжены Белорусским 7-ым гусарским полком (1803-1918) и Гродненским гусарским лейб-гвардии полком (1824-1918), 
		получившим свои названия, после комплектования личного состава уроженцами Беларуси.
		</p>
		<p>
		<b>Кстати</b>
		</p>
		<p>	
		Командиром Белорусского гусарского полка в 1850 году был А. А. Позняк. В этом полку служил Денис Давыдов.
		 </br>В Гродненском гусарском полку служил М. Ю. Лермонтов.
		</p>
			
		

		
		
		</br></br></br>
		<p><small>
		http://www.sb.by/kultura/article/dolgaya-doroga-domoy-3.html</br>
		http://nesvizh.by/390-oruzhie-radzivillov-v-rossiyskih-muzeyah.html</br>
		http://people.onliner.by/2015/05/05/dospehi/</br>
		be-x-old.wikipedia.org</br>
		be.wikipedia.org</br>
		pl.wikipedia.org</br>
		uk.wikipedia.org</br>
		ru.wikipedia.org</br>
		</small></p>
		
	</div>
	
	<!-- ***Sidebar Right*** -->

	<? include ("../_include/right_sidebar_terms.php"); ?>

	
</div>

<!-- ***Include Footer*** -->
<? include ("../_include/footer.php"); ?>